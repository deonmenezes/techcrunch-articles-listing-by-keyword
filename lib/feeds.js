...
//   cat    classifier hint appended to `categories`. Exact NewsCategory values
//          win immediately; legacy broad "Science" lets item tags/title choose
//          a science branch before using the biology fallback.
...
function label(src, art) {
  ...
  // App-category/classifier hint. Appended LAST so the outlet's own labels keep
  // driving the detailed science branch. Canonical hints such as Security win
  // exactly; legacy broad Science is only a fallback signal.
